...
CLIENT_SECRET = "XSPACE_CLIENT_SECRET_98"
ADMIN_SECRET_KEY = "ADMIN_SECRET_KEY"

# --- WATCHDOG CONFIG ---
MAX_BROADCAST_DURATION = 3 * 60 * 60
SILENCE_TIMEOUT = 10 * 60
...
DB_FILE = os.path.abspath("xspace.db")
...
def init_db():
    try:
        with get_db_connection() as conn:
            ...
            count = c.fetchone()[0]
            logger.info(f"💾 Database Loaded. Path: {DB_FILE}. Existing Scores: {count}")
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, target_lang: str = "English"):
        ...
        client_ip = "Unknown"
        if websocket.client: client_ip = websocket.client.host
    ...
    async def broadcast_result(self, original: str, translations: Dict[str, str]):
        ...
        for ws, lang in self.active_connections:
            try:
                text_to_send = ""
                if lang == "English": text_to_send = original
                elif lang in translations: text_to_send = translations[lang]
                if text_to_send:
                    payload = {
        ...
        if disconnected: await self.broadcast_stats()
...
class AudioStreamProcessor:
    ...
    async def stop_stream(self):
        if self.is_running: logger.info("🛑 STOPPING CURRENT STREAM & CLEANING RESOURCES...")
        self.is_running = False
    ...
    async def process_audio(self):
        ...
        except asyncio.CancelledError: logger.info("Processing task cancelled.")

    async def transcribe_and_distribute(self, wav_data, filename):
        try:
            transcription = await clients[0].audio.transcriptions.create(
                file=("audio.wav", wav_data), model="whisper-large-v3", response_format="json"
            )
...
def submit_score(sub: ScoreSubmission):
    ...
    for char in msg:
        ...
        expected_hash &= 0xFFFFFFFF
    if expected_hash > 0x7FFFFFFF: expected_hash -= 0x100000000
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info(f"🏆 NEW SCORE SAVED: {sub.name} - {sub.score}")
        return {"status": "ok"}
    except Exception as e:
...
def clear_scores(key: str):
    if key != ADMIN_SECRET_KEY: raise HTTPException(status_code=403, detail="Unauthorized")
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
    ...
    try:
        while True:
            ...
            if action == "join":
                ...
                if processor.current_title: await websocket.send_json({"type": "meta", "title": processor.current_title})
                if manager.history:
                     payload = []
                     for item in manager.history:
                         txt = item["original"] if target_lang == "English" else item["translations"].get(target_lang, "")
                         if txt: payload.append({"transcript": txt, "timestamp": item["timestamp"], "is_user": False})
                     if payload: await websocket.send_json({"type": "history", "data": payload})
            elif action == "stream":
                url = data.get("url")
                if url:
